...
> 💡 **它解决什么？** 与智能体对话，输入不该只有键盘打字。语音说出来、PDF 甩进来、提示词一键优化——**把更多种「输入」变成与智能体顺畅交谈的方式**。图片输入 DSH 原生已支持，BetterInput 专注把其余还没被官方覆盖的输入做得更好。这才是 BetterInput 的意义：不只是语音，是更好的输入。
...
### 媒体 & 文件
> 📷 图片输入：**DSH `rc.8` 起已原生支持**（粘贴 / 拖拽 / 选择文件），无需插件。由于 DSH API 已原生支持图片输入，我们**不再提供图片相关的插件功能**。BetterInput 专注官方尚未覆盖的方向。
